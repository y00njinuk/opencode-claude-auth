...
> Fork-only additions:
>
> - [`OPENCODE_CLAUDE_AUTH_PROACTIVE_REFRESH_MS`](#environment-variable-overrides) — tunes the background proactive refresh introduced upstream in 2.1.4. This fork ships it **off by default**, restoring the 2.0.0 policy where the background timer only mirrors credentials into `auth.json` and the request path does all refreshing. Set a positive value to opt back in.
...
| `OPENCODE_CLAUDE_AUTH_PROACTIVE_REFRESH_MS` | How far ahead of expiry the 5-minute background timer refreshes the token. `0` (the default) disables background refresh entirely — the timer then only keeps `auth.json` in sync and refreshing is left to the request path's 60s window. Set e.g. `3600000` to refresh an hour ahead of expiry. Invalid or negative values use the default. | `0` (disabled)                                                     |
